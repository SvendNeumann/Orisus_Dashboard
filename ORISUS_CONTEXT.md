...
- Lohnjournal / DATEV-Personalkosten:
  ...
  - Lohnjournal-PDF-Text sowie Ordner-/Dateiname `kallweit` gehoeren fachlich zum Standort Kirchberg und duerfen als Standort-Fallback genutzt werden, wenn die DATEV-Kopfzeile keinen App-Standort liefert. Wenn die Datei anders benannt ist, soll der PDF-Text-Fallback `Kirchberg` weiterhin greifen.
  - Weitere Lohnjournal-Standort-Aliase: `krauhausen` gehoert zu Huettenberg, `krause` gehoert zu Essen, `zorn de bulach`/`zdb` gehoert zu Kehl, `hangx` gehoert zu Ulmet. Diese Aliaslogik darf fuer Ordner-/Dateinamen und PDF-Text genutzt werden und hat Vorrang vor generischen Ortsnamen im PDF-Text, weil alte DATEV-PDFs teils abweichende Adress-/Standorttexte enthalten.
  - Beim Ordnerupload muessen versteckte macOS-Systemdateien wie `.DS_Store` oder `__MACOSX` ignoriert werden; sie duerfen nicht als fehlerhafte Lohnjournaldateien im Importbericht landen.
  - Aeltere DATEV-Lohnjournal-PDFs duerfen ueber die mehrseitige `Personalkostenuebersicht` gelesen werden: Die fachlich massgebliche Periodensumme steht auf der Uebersichtsseite in den Summenzeilen, nicht im Steuerblock `Summen aus Lohnabrechnung`. Zu verwenden sind `Gesamtkosten ohne Erstatt.` und `Gesamtkosten inkl. Erstatt.`; die Differenz wird als Erstattung ausgewiesen. Wenn die Browser-PDF-Auslese die Einzelzeilen spaltenweise zerlegt, muessen diese Periodensummen die kleinteilig erkannten Einzelabrechnungswerte ueberschreiben.
  - Fix 27.06.2026: Dieser Fallback auf die DATEV-`Personalkostenuebersicht` muss auch greifen, wenn die Datei bereits ueber Entgeltabrechnungsseiten erkannt wurde. Das betrifft insbesondere Kirchberg/Kallweit ab Juli 2024 bis vor der modernen Personalkostenuebersicht; sonst bleiben die Periodensummen faelschlich bei kleinteiligen Werten wie 14.607 EUR haengen. Bei mehrseitigen Personalkostenuebersichten muss die groesste erkannte Uebersichtssumme verwendet werden, damit z. B. Jan 2026 nicht auf die Einzelzeile Mario Kallweit 32.912,06 EUR faellt, sondern auf die Summenzeile 122.538,62 EUR / 120.866,48 EUR inkl. Erstattung. Weitere gepruefte Beispiele: Juli 2024 61.233,37 EUR / 59.024,73 EUR inkl. Erstattung; August 2024 69.681,80 EUR / 65.134,51 EUR inkl. Erstattung; Maerz 2025 87.099,17 EUR / 81.814,48 EUR inkl. Erstattung.
  - Im Altformat muessen Mitarbeiter-Namen aus den Einzelabrechnungsseiten gegen Adress-, Praxis-, Bank- und Krankenkassenzeilen abgegrenzt werden. Auseinandergezogene DATEV-Namen wie `A h m a d A l m o h a m a d` sind wieder zu normalen Namen zusammenzusetzen.
...
- Orisus-Dashboard:
  ...
  - Liquiditaets-/Kontostand-Kacheln zeigen, sofern `Kontostand Monatsende` aus Bank-/Finanzdaten vorhanden ist, klein den Vormonats-Kontostand und die Abweichung zum aktuellen Stand. Offene-Forderungen-Kacheln nutzen analog eine optionale Forderungs-Monatshistorie aus expliziten CFO-Importzeilen (`Offene Forderungen gesamt` bzw. `noch nicht geflossen gesamt`) und zeigen Vormonatsbestand plus Abweichung nur, wenn diese Monatsdaten im bestaetigten Import vorhanden sind. Alte bestaetigte CFO-Importe ohne diese optionale Historie bleiben gueltig; bis zum erneuten CFO-Import zeigen die Forderungskacheln einen kleinen Hinweis `Vormonat: nach CFO-Neuimport verfügbar` statt eines falschen 0-Werts.
  ...
  - Neuer Personal-Tab `Personalkosten Lohnjournal` ist als vorstandstaugliche Managementsicht aufgebaut: KPI-Karten oben, Verlauf mit BWA-Abgleich, Management-Fokus, Standortvergleich, Fluktuation, groesste BWA-Abweichungen, Behandlerkosten nach Arzt/PZR und kompakter Mitarbeiter-Fokus. Er soll keine zweite vollstaendige Mitarbeiterliste duplizieren; die Mitarbeiterliste bleibt der Stammdaten-/Detail-Tab.
  - Im Management-Fokus des Tabs `Personalkosten Lohnjournal` bedeutet `Ø MA-Kosten alle`: alle Lohnjournal-Gesamtkosten inkl. Erstattungen geteilt durch alle Mitarbeiterzeilen im ausgewaehlten Zeitraum; nicht nur Aerzte/Behandler. Dieser Wert wird als voller Eurobetrag mit Tausendertrennung angezeigt.
  - In allen Personal-Kosten-Auswertungen muss sichtbar sein, welche Datenbasis genutzt wird: Mitarbeiterliste/Personalimport fuer Stammdaten, FTE, AG-Aufwand und Fehlzeiten; DATEV-Lohnjournal fuer echte abgerechnete Personalkosten; BWA-Personalkosten bzw. Honorarumsatz nur als klar benannter Vergleich. Wenn eine Auswertung das Lohnjournal nutzt, darf dies nicht wie ein Wert aus der Mitarbeiterliste wirken.
  - Im `Personal-Cockpit` muessen die oberen KPI-Kacheln die Datenbasis direkt in der Kachel zeigen: Aktive Mitarbeiter, FTE, Neueinstellungen, Fluktuation, Wochenstunden, AG-Aufwand und Krankheit basieren auf Mitarbeiterliste/Personalimport; `Ø Gesamtleistung je FTE` kombiniert CFO-BWA-Gesamtleistung mit FTE aus dem Personalimport. Diese Cockpit-KPIs sind kein DATEV-Lohnjournal, ausser es steht ausdruecklich dort.
  - Im `Personal-Cockpit` muessen auch Tabellen wie `Personalstruktur`, `Kostenuebersicht` und `Kosten & operative Kennzahlen` sichtbar ausweisen, dass sie aus Mitarbeiterliste/Personalimport abgeleitet sind. FTE = Wochenstunden / 40; AG-Aufwand stammt aus dem Personalimport; Krankheit aus Fehlzeitenimport; Fluktuation aus Mitarbeiterstamm/Austritten. Kein DATEV-Lohnjournal, ausser dies wird ausdruecklich als Quelle genannt.
  - Im `Personal-Cockpit` muessen auch Charts und Ranglisten wie aktive Mitarbeiter je Standort, Krankheitstage je Standort, AG-Kosten je Standort und Top-Krankheitstage sichtbar die Quelle nennen: Mitarbeiterliste/Personalimport bzw. Fehlzeitenimport; kein DATEV-Lohnjournal, ausser ausdruecklich benannt.
  - Im Tab `Personalkosten Lohnjournal` werden echte Lohnjournal-Behandlerkosten gegen reinen Honorarumsatz aus `Input_Behandler_Leistung` gematcht. Arzt und PZR werden getrennt gezeigt; die Spalte Monate bleibt in dieser Vorstandssicht ausgeblendet. Es werden nur aktive Lohnjournal-Mitarbeiter im jeweiligen Monat beruecksichtigt. Dietrich, Schneider, Pomsel und Korkel sind in dieser Behandlerkostenquote ausgeschlossen.
  - Der Block `Behandlerkosten vs. Honorarumsatz` hat einen eigenen Standortfilter mit `Alle Standorte`. Diese Auswertung muss immer ueber die gesamte geladene Lohnjournalperiode seit Vertragsstart laufen und darf nicht vom oberen Jahresfilter abhaengen, weil Provisionen/Behandlerkosten jahresuebergreifend ausgezahlt werden koennen.
  - Die Tabelle `Fluktuation aus Lohnjournal` zeigt als Mitarbeiterbasis den durchschnittlichen Monatsbestand aus den Lohnjournalmonaten je Standort, nicht die kumulierte Anzahl aller jemals im Zeitraum gefundenen Personalnummern/Namen. Ein-/Austritte bleiben eindeutige Personalnummern im Zeitraum; Austrittsquote = Austritte / Ø Monatsbestand.
  - Wenn Lohnjournal-Namen und Behandlernamen nicht direkt identisch sind, darf der Arztkosten-Match je Standort ueber eindeutig gleiche Nachnamen erfolgen. Dabei sollen sowohl DATEV-Schreibweisen `Nachname Vorname` als auch Behandler-Schreibweisen mit Titel/Initiale beruecksichtigt werden; zusammengesetzte Nachnamen wie `Al Sare` muessen ebenfalls erkannt werden. Bei mehreren Treffern bleibt der Match offen.
  - Manuelle Lohnjournal-Namenskorrektur: Kirchberg Personalnummer `01016` ist `Franziska Paatsch`. Wenn alte DATEV-Uebersichtsseiten oder gespeicherte Daten nur `FP`/Initialen zeigen, muss die App diesen Namen in Auswertungen als `Franziska Paatsch` anzeigen.
  - Im `Mitarbeiter-Fokus aus Lohnjournal` duerfen reine Initialen-Platzhalter wie `FP`/`F P` nicht als eigene Top-Kostenposition erscheinen. Diese Ausblendung betrifft nur die Fokusliste, nicht die importierten Lohnjournal-Summen.
