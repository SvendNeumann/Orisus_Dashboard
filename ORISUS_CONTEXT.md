...
- Rollen-/Sicherheitspruefung:
  ...
  - Oben im Untertab `Allgemeine Informationen` stehen zwei zeitraumfilterbare KPI-Kacheln: `Gesamtumsatz gem. BWA` und `Kumuliertes EBITDA`. Beide Werte werden aus den BWA-Zeilen `summe_umsatz` bzw. `ebitda` je Standort im gewaehlten Zeitraum innerhalb der jeweiligen Vertragsperiode summiert; Info-Buttons zeigen die Standort-Herleitung.
