...
- Lohnjournal / DATEV-Personalkosten:
  ...
  - Aeltere DATEV-Lohnjournal-PDFs ohne moderne `Personalkostenuebersicht` duerfen ueber einen Altformat-Fallback gelesen werden: Mitarbeiter, Brutto und Arbeitgeber-Zusatzkosten kommen aus den Einzelabrechnungsseiten, AAG-Erstattungen aus der Erstattungsuebersicht. Wenn die Browser-PDF-Auslese die Einzelzeilen spaltenweise zerlegt und dadurch ein offensichtlich zu niedriges Brutto entsteht, darf das Brutto aus dem alten DATEV-Block `Summen aus Lohnabrechnung` als Sicherheitsfallback genutzt werden. Dabei muss ein plausibler grosser Bruttowert aus dem Summenblock genutzt werden, nicht eine kleinteilige Folgezahl und niemals ein Datumswert wie `21.02.2025`. Der Importbericht weist diesen Fallback als neutralen Hinweis aus.
  - Fix 27.06.2026: Dieser Sicherheitsfallback aus `Summen aus Lohnabrechnung` muss auch greifen, wenn die Datei bereits ueber Entgeltabrechnungsseiten erkannt wurde. Das betrifft insbesondere Kirchberg/Kallweit ab Juli 2024 bis vor der modernen Personalkostenuebersicht; sonst bleibt das Gesamtbrutto faelschlich bei einem kleinteiligen Wert wie 14.607 EUR haengen.
