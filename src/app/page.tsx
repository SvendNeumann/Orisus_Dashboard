...
const bankMovementKeyAliases: Record<string, string[]> = {
  ...
  davon_praxisumsatz: ["ist_geldeingang_bank"],
    ...
    <div className="rounded-md border border-border bg-white p-3 text-sm shadow-lg">
      ...
      {row.abweichungUebernahme != null && row.abweichungUebernahmePct != null ? (
        <p className={cn("font-bold", row.abweichungUebernahme < 0 ? "text-red-700" : "text-emerald-700")}>
          Abw. Übernahme: {eur(row.abweichungUebernahme)} ({pct(row.abweichungUebernahmePct)})
        </p>
      ) : null}
    </div>
  );
...
function ebitdaTargetChartRow(site: DashboardSite, importedData?: ImportedDashboardData | null): EbitdaTargetChartRow {
  ...
  const takeoverTargetPa = site.darlehen.zielEbitdaUebernahme ?? 0;
  const fallbackUebernahmeMonthly = takeoverTargetPa ? takeoverTargetPa / 12 : 0;
...
type EarlyWarningRow = {
  ...
  siteName: string;
  monthLabel: string;
  category: "Ergebnis" | "Kosten" | "Cash/Bank" | "Personal" | "Patienten" | "Datenbasis";
  signal: Status;
  title: string;
  finding: string;
  source: string;
...
function Fruehwarnsystem({
  ...
  const monthValue = (siteId: string, metricKey: string, month: number) => {
    const row = bwaRow(siteId, metricKey);
    const key = `${year}-${month}`;
    return row?.hasValueByMonth[key] || row?.hasDataByMonth[key] ? row.valuesByMonth[key] ?? 0 : null;
  };
  const previousMonthValue = (siteId: string, metricKey: string, month: number) => {
    const row = bwaRow(siteId, metricKey);
    const key = `${year - 1}-${month}`;
    return row?.hasValueByMonth[key] || row?.hasDataByMonth[key] ? row.valuesByMonth[key] ?? 0 : null;
  };
  const ratioOrNull = (value: number | null, basis: number | null) =>
    value != null && basis != null && Math.abs(basis) > 0 ? (value / basis) * 100 : null;
  const costQuote = (siteId: string, month: number, previous = false) => {
    const value = (metricKey: string) => previous ? previousMonthValue(siteId, metricKey, month) : monthValue(siteId, metricKey, month);
    const revenue = value("summe_umsatz");
    const ebitda = value("ebitda");
    if (revenue == null || ebitda == null || Math.abs(revenue) <= 0) return null;
    return Math.max(0, ((revenue - ebitda) / revenue) * 100);
  };
  const personalQuote = (siteId: string, month: number, previous = false) => {
    const value = (metricKey: string) => previous ? previousMonthValue(siteId, metricKey, month) : monthValue(siteId, metricKey, month);
    return ratioOrNull(Math.abs(value("personalkosten_gesamt") ?? 0), value("summe_umsatz"));
  };
  const patientMonthValue = (siteId: string, metricKey: PatientMetricKey, month: number, targetYear = year) => {
    const row = patientMetricRow(importedData.patientRows, siteId, metricKey);
    const key = `${targetYear}-${month}`;
    if (!row || !Object.prototype.hasOwnProperty.call(row.valuesByMonth, key)) return null;
    const value = row.valuesByMonth[key];
    return Number.isFinite(value) ? value : null;
  };
  ...
  activeSites.forEach((site) => {
    ...
    const hasTwoConsecutiveTargetMisses =
      lastUnderTarget != null && underTargetMonths.includes(lastUnderTarget - 1);
    if (hasTwoConsecutiveTargetMisses) {
      ...
      warnings.push({
        ...
        action: "Standort in Ergebnisreview nehmen: Umsatzmix, Kostenblock und Behandlerauslastung prüfen."
      });
    }
    ...
    availableMonths.forEach((month) => {
      ...
      if (currentCostQuote != null && previousCostQuote != null && currentCostQuote - previousCostQuote >= 5) {
        warnings.push({
          ...
          action: "Kostenentwicklung nach Material, Fremdlabor, Personal und sonstigen Kosten aufbrechen."
        });
      }
      ...
      if (
        ...
        revenueDeclinePct <= -5 &&
        currentPersonalQuote != null &&
        previousPersonalQuote != null &&
        currentPersonalQuote - previousPersonalQuote >= 3
      ) {
        warnings.push({
          id: `${site.id}-${month}-personalquote`,
          siteName: site.name,
          monthLabel,
          category: "Personal",
          signal: currentPersonalQuote - previousPersonalQuote >= 7 ? "red" : "yellow",
          title: "Personalkostenquote steigt bei sinkendem Umsatz",
          finding: `Umsatz ${pct(revenueDeclinePct)} ggü. Vorjahresmonat, Personalkostenquote +${pct(currentPersonalQuote - previousPersonalQuote)}-Pkt.`,
          source: "BWA: Summe Umsatz und Personalkosten gesamt",
          action: "Dienstplanung, Behandlerkapazität und Umsatz je FTE prüfen."
        });
      }
      ...
      if (rooms && treated != null && previousTreated != null && previousTreated > 0) {
        ...
        const previousPatientsPerRoom = previousTreated / rooms;
        const decline = ((currentPatientsPerRoom - previousPatientsPerRoom) / previousPatientsPerRoom) * 100;
        if (decline <= -10) {
          warnings.push({
            id: `${site.id}-${month}-patients-room`,
            siteName: site.name,
            monthLabel,
            category: "Patienten",
            signal: decline <= -20 ? "red" : "yellow",
            title: "Patienten je Zimmer sinken",
            finding: `Patienten je Zimmer ${pct(decline)} ggü. Vorjahresmonat.`,
            source: "Patientenimport: behandelte Patienten / hinterlegte Behandlungszimmer",
            action: "Auslastung, Recall, Neupatientenzufuhr und Terminbuchung im Standort prüfen."
          });
        }
      }
      ...
      if (missed != null && booked && previousMissed != null && previousBooked) {
        ...
        if (increase >= 3) {
          warnings.push({
            id: `${site.id}-${month}-cancellation`,
            siteName: site.name,
            monthLabel,
            category: "Patienten",
            signal: increase >= 7 ? "red" : "yellow",
            title: "Terminausfallquote erhöht ggü. Vorjahr",
            finding: `Ausfallquote ${pct(currentCancellationRate)} liegt ${pct(increase)}-Pkt. über Vorjahresmonat.`,
            source: "Patientenimport: ausgefallene Termine / gebuchte Termine",
            action: "No-Show-Prozess, Recall-Kommunikation und Terminbestätigung prüfen."
          });
        }
      }
    });
    ...
    if (latestMonth) {
      ...
      if (!hasLatestBwa || !hasLatestPatients) {
        warnings.push({
          ...
          action: "Wenn Daten in Excel vorliegen, CFO-Import neu bestätigen; sonst als offene Datenbasis markieren."
        });
      }
    }
    ...
    <section className="space-y-5">
      ...
      <Card className="overflow-hidden">
        <div className="border-b border-border p-4">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Jede Zeile nennt Auslöser, Quelle und nächsten Prüfschritt. Fehlende Daten werden nicht als Null interpretiert.
          </p>
        </div>
        {sortedWarnings.length ? (
          <div className="overflow-x-auto">
            <table className="data-table border-separate border-spacing-0 text-sm">
              <thead>
                <tr>
                  {["Signal", "Standort", "Zeitraum", "Datenwelt", "Warnung", "Befund", "Quelle", "Nächster Schritt"].map((head) => (
              ...
              <tbody>
                {sortedWarnings.map((warning) => (
                  <tr key={warning.id}>
                    ...
                    <td className="border-b border-r border-border p-3 font-bold">{warning.siteName}</td>
                    <td className="border-b border-r border-border p-3">{warning.monthLabel}</td>
                    <td className="border-b border-r border-border p-3">{warning.category}</td>
                    <td className="border-b border-r border-border p-3 font-bold">{warning.title}</td>
                    <td className="border-b border-r border-border p-3">{warning.finding}</td>
                    <td className="border-b border-r border-border p-3 text-sm text-muted-foreground">{warning.source}</td>
                    <td className="border-b border-r border-border p-3">{warning.action}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function Analysen({
  ...
  const scorecardRows = patientSiteRows.map((row) => {
    ...
    const bankRevenueRow = bankRowFor("davon_praxisumsatz") ?? bankRowFor("geldeingang_bank_gesamt");
    const bankRevenue = Math.max(0, Math.abs(bankMovementValueForPeriod(bankRevenueRow, "Gesamte Periode")) + (manualAdjustment?.revenue ?? 0));
  ...
  const openBenchmarkingReport = () => {
    ...
    openPrintableReport(
      ...
      buildReportDocument({
        ...
        subtitle: `Zeitraum: ${period} | Vergleich: ${comparison} | Ansicht: ${viewMode}. Kompakter Report mit Standort-vs.-Ø-Orisus-Vergleich, Quellen, Rechenbasis und ergänzenden Detailtabellen.`,
...
function benchmarkOverviewReportFormat(value: number | null, type: BenchmarkOverviewRow["type"]) {
  if (value == null || !Number.isFinite(value)) return "n. v.";
...
function benchmarkOverviewReportStatus(row: BenchmarkOverviewRow): Status | "neutral" {
  ...
  const tolerance = row.type === "currency" ? Math.max(Math.abs(row.orisusValue) * 0.03, 5000) : 0.5;
...
function benchmarkOverviewReportTable(rows: BenchmarkOverviewRow[]) {
  return `<table class="report-table compact benchmark-overview-table">
    ...
    <tbody>${rows.map((row) => {
      const status = benchmarkOverviewReportStatus(row);
      return `<tr>
        <td class="label-cell">${reportEscape(row.group)}</td>
        <td>${reportEscape(row.label)}</td>
        <td>${reportEscape(benchmarkOverviewReportFormat(row.siteValue, row.type))}</td>
...
const bankBwaReconciliationManualAdjustments: Record<string, { revenue: number; practiceCosts: number; note: string }> = {
  ulmet: {
    revenue: -100000,
    practiceCosts: 0,
    note: "-100.000 EUR Bank-Praxisumsatz: interner operativer Kredit von Kirchberg herausgerechnet."
  },
  kirchberg: {
    revenue: 0,
...
function CashflowCostReconciliation({ sites = standorte, importedData }: { sites?: DashboardSite[]; importedData?: ImportedDashboardData | null }) {
  ...
  if (!importedData?.bwaRows?.length || !bankRows.length) {
    return (
      <Card className="p-4">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Nach dem nächsten bestätigten CFO-Import werden hier je Standort Bank-Praxisumsatz gegen BWA-Umsatz und Bank-Praxisausgaben gegen operative BWA-Kosten plus Investitionen über die gesamte Vertragsperiode plausibilisiert.
        </p>
      </Card>
    );
  }
  ...
  const reconciliationSignal = (deviation: number, basis: number): Status => {
    const absoluteDeviation = Math.abs(deviation);
    return absoluteDeviation <= Math.max(50000, basis * 0.1)
      ? "green"
      : absoluteDeviation <= Math.max(100000, basis * 0.2)
        ? "yellow"
        : "red";
  };
  const comparisonRows = activeSites.map((site) => {
    ...
    const bankRevenueRow = rowFor(site.id, "davon_praxisumsatz") ?? rowFor(site.id, "geldeingang_bank_gesamt");
    const rawBankRevenue = Math.abs(bankMovementValueForMonthKeys(bankRevenueRow, monthKeys));
    ...
    const costDeviation = bankPracticeCosts - bwaCostBase;
    const costDeviationPct = bwaCostBase ? (costDeviation / bwaCostBase) * 100 : 0;
    const revenueSignal = reconciliationSignal(revenueDeviation, bwaRevenue);
    const costSignal = reconciliationSignal(costDeviation, bwaCostBase);
    const signal: Status = revenueSignal === "red" || costSignal === "red" ? "red" : revenueSignal === "yellow" || costSignal === "yellow" ? "yellow" : "green";
    ...
    return {
      ...
      revenueSourceLabel: bankRevenueRow && matchesBankMovementKey(bankRevenueRow.label, "davon_praxisumsatz") ? "Bank-Praxisumsatz" : "Bankeinnahmen gesamt",
      adjustmentNote: manualAdjustment?.note ?? ""
    };
  });
  const totals = comparisonRows.reduce(
    (sum, row) => ({
      bankRevenue: sum.bankRevenue + row.bankRevenue,
      bwaRevenue: sum.bwaRevenue + row.bwaRevenue,
      revenueDeviation: sum.revenueDeviation + row.revenueDeviation,
      ...
      costDeviation: sum.costDeviation + row.costDeviation
    }),
    { bankRevenue: 0, bwaRevenue: 0, revenueDeviation: 0, bankPracticeCosts: 0, operatingCosts: 0, investments: 0, bwaCostBase: 0, costDeviation: 0 }
  );
  ...
  const largestDeviation = comparisonRows
    .flatMap((row) => [
      { site: row.site.name, label: "Praxisumsatz", value: row.revenueDeviation },
  ...
  return (
    <Card className="overflow-hidden">
      <div className="table-head flex flex-col gap-3 p-4 text-white sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-white/75">
            BWA gibt den Zeitraum vor: Bank-Praxisumsatz und Bank-Praxisausgaben werden nur gegen dieselben BWA-Monate gerechnet.
          </p>
        </div>
      ...
      <div className="border-b border-border bg-slate-50 p-3 text-sm text-muted-foreground">
        Abgleichslogik je Standort auf Basis der vorhandenen BWA-Monate. Wenn Bankdaten bereits weiter reichen als die BWA, werden die spaeteren Bankmonate hier nicht einbezogen. Einnahmenseite = Bank-Praxisumsatz vs. BWA-Umsatz. Kostenseite = Praxisausgaben gem. Bankbewegung vs. operative BWA-Kosten bis EBITDA plus Investitionen. Tilgung, Zins, Intercompany und sonstige Bankeingaenge sind hier bewusst nicht enthalten.
        Fix hinterlegte Bereinigung: Ulmet -100.000 EUR Bank-Praxisumsatz und Kirchberg -100.000 EUR Praxisausgaben wegen internem operativem Kredit Kirchberg an Ulmet.
      </div>
      <div className="grid gap-3 border-b border-border p-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Mini label="Abw. Praxisumsatz gesamt" value={`${eur(totals.revenueDeviation)} | ${pct(totalRevenueDeviationPct)}`} />
          ...
          <tbody>
            {comparisonRows.map((row) => (
              <tr key={row.site.id}>
                ...
                <TableCell>
                  <span className="block">{eur(row.bankRevenue)}</span>
                  {row.revenueSourceLabel !== "Bank-Praxisumsatz" ? <span className="block text-[10px] text-muted-foreground">Fallback: {row.revenueSourceLabel}</span> : null}
                </TableCell>
                ...
                <TableCell tone={Math.abs(row.revenueDeviationPct) > 20 ? "red" : Math.abs(row.revenueDeviationPct) <= 10 ? "green" : undefined}>{pct(row.revenueDeviationPct)}</TableCell>
                ...
                <TableCell tone={row.costDeviation > 0 ? "red" : row.costDeviation < 0 ? "green" : undefined}>{eur(row.costDeviation)}</TableCell>
                <TableCell tone={Math.abs(row.costDeviationPct) > 20 ? "red" : Math.abs(row.costDeviationPct) <= 10 ? "green" : undefined}>{pct(row.costDeviationPct)}</TableCell>
                <TableCell>
            ...
            <tr className="summary-row">
              ...
              <TableCell strong summary tone={Math.abs(totalRevenueDeviationPct) > 20 ? "red" : Math.abs(totalRevenueDeviationPct) <= 10 ? "green" : undefined}>{pct(totalRevenueDeviationPct)}</TableCell>
              ...
              <TableCell strong summary tone={totals.costDeviation > 0 ? "red" : totals.costDeviation < 0 ? "green" : undefined}>{eur(totals.costDeviation)}</TableCell>
              <TableCell strong summary tone={Math.abs(totalCostDeviationPct) > 20 ? "red" : Math.abs(totalCostDeviationPct) <= 10 ? "green" : undefined}>{pct(totalCostDeviationPct)}</TableCell>
              <TableCell summary>{""}</TableCell>
            </tr>
          </tbody>
        </table>
  ...
  return christianHenriciInfoRows(sites).flatMap((infoRow) => {
    ...
    return years
      .map((year) => {
        ...
        const targetMonths = christianHenriciActiveBwaMonthsInYear(ebitdaRow, site.start, year);
        const targetEbitdaKvYear = Math.round((infoRow.targetEbitdaKvPa / 12) * targetMonths);
...
function christianHenriciBwaBreakdown(sites: DashboardSite[], importedData: ImportedDashboardData, metricKey: string, period: string) {
  return christianHenriciInfoRows(sites).map((infoRow) => {
    const metricRow = importedData.bwaRows.find((row) => row.siteId === infoRow.site.id && row.metricKey === metricKey);
    const value = importedPeriodValueWithinContract(metricRow, period, infoRow.site.start) ?? 0;
    return {
      site: infoRow.site,
      value: Math.round(value)
    };
...
function christianHenriciContractMonthsInYear(start: string, year: number) {
  ...
  if (year === parts.year) return 12 - parts.month + 1;
  return 12;
}

function christianHenriciActiveBwaMonthsInYear(row: ImportedBwaRow | undefined, start: string, year: number) {
  const activeMonths = Array.from({ length: 12 }, (_, index) => index + 1).filter((month) =>
    isPeriodOnOrAfterStart(year, month, start) &&
    Boolean(row?.hasValueByMonth?.[`${year}-${month}`] || row?.hasDataByMonth?.[`${year}-${month}`])
  );
...
function ChristianHenriciInfo({ sites, importedData }: { sites: DashboardSite[]; importedData: ImportedDashboardData }) {
  ...
  }, [importedData, periodOptions, revenuePeriod, ebitdaPeriod, annualYear, availableAnnualYears]);
    ...
    <section className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <KpiCard
          ...
          status="green"
          control={
            <Select value={revenuePeriod} onChange={(event) => setRevenuePeriod(event.target.value)}>
          ...
          info={
            <div className="space-y-3">
              <p>
                Summe der BWA-Gesamtleistung bzw. `Summe Umsatz` im ausgewählten Zeitraum.
                Die Werte werden je Standort nur innerhalb der jeweiligen Vertragsperiode berücksichtigt.
              </p>
              <InfoTextLine label="Zeitraum" value={performancePeriodLabel(revenuePeriod)} strong />
              <div className="space-y-1">
                {revenueBreakdown.map((row) => <InfoLine key={row.site.id} label={row.site.name} value={row.value} />)}
                <InfoLine label="= Gesamtumsatz gem. BWA" value={revenueTotal} strong />
              </div>
            </div>
          }
        />
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-white/80">
                ...
                Ziel-EBITDA wird auf die aktiven BWA-Monate des jeweiligen Jahres ab Vertragsstart gerechnet.
              </p>
            </div>
            <label className="grid min-w-48 gap-1 text-xs font-bold uppercase text-white/80">
              Jahr
              <Select value={annualYear} onChange={(event) => setAnnualYear(event.target.value)}>
                <option value="alle">Alle Jahre</option>
                {availableAnnualYears.map((year) => <option key={year} value={String(year)}>{year}</option>)}
              </Select>
            </label>
          </div>
        </div>
            ...
            <tbody>
              {annualRows.length ? annualRows.map((row) => (
                <tr key={`${row.site.id}-${row.year}`} className="border-t border-border">
                  ...
                  <td className="border-r border-border p-3 text-right">{row.targetEbitdaKvYear ? eur(row.targetEbitdaKvYear) : "-"}</td>
                  <td className="border-r border-border p-3 text-right">{row.targetMonths}</td>
...
function buildPmrBenchmarkPage(
  ...
  const averages = {
    ...
    personalquote: pmrBenchmarkAverage(comparisonRows.map((row) => row.personalquote)),
    sonstigeKostenquote: pmrBenchmarkAverage(comparisonRows.map((row) => row.sonstigeKostenquote)),
    gesamtkostenquote: pmrBenchmarkAverage(comparisonRows.map((row) => row.gesamtkostenquote))
  };
  const overviewRows: BenchmarkOverviewRow[] = [
    {
      group: "Ergebnis",
      label: "EBITDA-Marge",
      siteValue: selectedRow?.ebitdaMargin ?? null,
      orisusValue: averages.ebitdaMargin,
      type: "percent",
      higherIsBetter: true,
      source: "BWA"
    },
    {
      ...
      label: "Personalkostenquote",
      siteValue: selectedRow?.personalquote ?? null,
      orisusValue: averages.personalquote,
      type: "percent",
      higherIsBetter: false,
      source: "BWA"
    },
    {
      ...
      label: "Materialquote",
      siteValue: selectedRow?.materialquote ?? null,
      orisusValue: averages.materialquote,
      type: "percent",
      higherIsBetter: false,
      source: "BWA"
    },
    {
      ...
      label: "Fremdlaborquote",
      siteValue: selectedRow?.fremdlaborquote ?? null,
      orisusValue: averages.fremdlaborquote,
      type: "percent",
      higherIsBetter: false,
      source: "BWA"
    },
    {
      ...
      label: "Gesamtkostenquote",
      siteValue: selectedRow?.gesamtkostenquote ?? null,
      orisusValue: averages.gesamtkostenquote,
      type: "percent",
      higherIsBetter: false,
      source: "BWA"
    },
    ...
    },
    {
      ...
      siteValue: selectedRow?.pvsPerOpeningHour ?? null,
      orisusValue: averages.pvsPerOpeningHour,
      type: "currency",
      higherIsBetter: true,
      source: "PVS / Öffnungszeiten"
    },
    {
      group: "Patienten",
    ...
    },
    {
      group: "Patienten",
      label: "Neupatientenquote",
      siteValue: selectedRow?.newPatientRate ?? null,
      orisusValue: averages.newPatientRate,
      type: "percent",
      higherIsBetter: true,
      source: "Patienten"
    },
    {
      ...
      siteValue: selectedRow?.attendanceRate ?? null,
      orisusValue: averages.attendanceRate,
      type: "percent",
      higherIsBetter: true,
      source: "Patienten"
    },
    {
      group: "Patienten",
      label: "Terminausfall",
      siteValue: selectedRow?.cancellationRate ?? null,
      orisusValue: averages.cancellationRate,
      type: "percent",
      higherIsBetter: false,
      source: "Patienten"
    }
  ];
  const overviewTable = benchmarkOverviewReportTable(overviewRows);
  const insightCards = [
    {
      label: "Umsatz je Öffnungsstunde",
